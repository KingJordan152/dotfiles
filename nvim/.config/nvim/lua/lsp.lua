...
vim.api.nvim_create_autocmd("LspAttach", {
  ...
  callback = function(event)
    ...
    vim.keymap.set(
      ...
      { desc = "Hover Documentation", buf = buf }
    )
    ...
    if client:supports_method("textDocument/codeAction") then
      ...
      if type(codeActionProvider) == "table" and client.server_capabilities.codeActionProvider["codeActionKinds"] ~= nil then
    ...
    if client:supports_method("textDocument/documentColor") then
      vim.keymap.set({ "n", "x" }, "grc", vim.lsp.document_color.color_presentation, { desc = "Select a different color presentation", buf = buf })
    end
    ...
    if client:supports_method("textDocument/inlayHint") then
      vim.keymap.set(
        ...
        { desc = "Toggle Inlay Hints", buf = buf }
      )
    end
