...
local function plugin_autocompletion(match)
  return vim.iter(vim.pack.get()):map(function(pack) return pack.spec.name end):filter(function(pack) return pack:find(match) end):totable()
end

---Returns a list of plugins that are currently *active*.
---@return table
local function get_active_plugins()
  return vim.iter(vim.pack.get()):filter(function(pack) return pack.active end):map(function(pack) return pack.spec.name end):totable()
end

--- Returns a list of plugins that are currently *inactive*.
---@return table
local function get_inactive_plugins()
  return vim.iter(vim.pack.get()):filter(function(pack) return not pack.active end):map(function(pack) return pack.spec.name end):totable()
end
...
user_command("PackDelete", function(args) vim.pack.del(args.fargs, { force = args.bang }) end, {
  desc = "Delete plugins",
  nargs = "+",
  bang = true,
  complete = plugin_autocompletion,
...
user_command("PackRestore", function() vim.pack.update(nil, { target = "lockfile" }) end, {
...
user_command("PackCount", function()
  ...
  vim.notify(string.format("%d total plugins are installed (%d are inactive)", #active + #inactive, #inactive), vim.log.levels.INFO)
